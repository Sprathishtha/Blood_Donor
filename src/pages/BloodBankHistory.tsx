...
//       .eq("user_id",authId)
//       .eq("role","blood_bank")
//       .neq("status", "pending")  // 👈 past records
...
//   units:n.blood_requests?.bloodbank_units || 0
...
export function BloodBankHistory() {
  ...
  const loadHistory = async () => {
    ...
    const formatted: HistoryItem[] = (data || []).map((n: any) => ({
      id: n.id,
      blood_group: n.blood_requests?.blood_group,
      quantity: n.blood_requests?.bloodbank_units || 0,
      hospital_name: n.blood_requests?.hospitals?.name || "-",
      status: n.status,
      created_at: n.created_at
    }))
