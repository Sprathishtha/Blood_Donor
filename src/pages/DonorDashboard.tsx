...
//   (!userProfile?.next_available_at ||
//     new Date(userProfile.next_available_at) <= new Date());
...
//            isAvailable ? 'border-blue-500' : 'border-gray-300'
//           }`}>
//             <div className="flex items-center justify-between">
//               <div>
...
//                 </p>
//               </div>
// {!isAvailable && userProfile?.next_available_at && (
//   <p className="text-xs text-red-500 mt-1">
//     Available after{" "}
//     {format(new Date(userProfile.next_available_at), 'MMM d, yyyy')}
//   </p>
// )}
//               {userProfile?.is_available
...
import { useAuth } from '../contexts/AuthContext';
import { supabase } from '../lib/supabase';
import { MapPin, Phone, Mail, Calendar, Droplet, CheckCircle, XCircle, AlertCircle, User } from 'lucide-react';
import { differenceInDays, format } from 'date-fns';
...
export function DonorDashboard() {
  ...
  useEffect(() => {
    ...
    const checkEligibility = async () => {
      ...
      if (!userProfile?.next_available_at) return;

      const today = new Date();
      const nextDate = new Date(userProfile.next_available_at);
  ...
  return (
    ...
    <AppLayout>
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-8">
          ...
          <div className="bg-white rounded-xl shadow-sm p-6 border-l-4 border-red-500">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-600">Blood Group</p>
                <p className="text-3xl font-bold">{userProfile?.blood_group}</p>
              </div>
              <Droplet className="w-12 h-12 text-red-500" />
            </div>
          </div>
          ...
          {/* Status */}
          <div className={`bg-white rounded-xl shadow-sm p-6 border-l-4 ${
            isAvailable ? 'border-blue-500' : 'border-red-400'
          }`}>
            <div className="flex justify-between">
              <div>
                <p className="text-sm text-gray-600">Status</p>
                ...
                {isAvailable ? (
                  <p className="text-xl font-bold text-blue-600">Available</p>
                ) : (
                  <>
                    <p className="text-xl font-bold text-red-500">Unavailable</p>
                    {userProfile?.next_available_at && (
                      <p className="text-xs text-gray-500 mt-1">
                        Available after {format(new Date(userProfile.next_available_at), 'MMM d, yyyy')}
                      </p>
                    )}
                  </>
                )}
        ...
        {/* ✅ PROFILE DETAILS (ADDED BACK + NAME) */}
        <div className="bg-white rounded-xl shadow-sm p-6 mb-6">
        ...
        <div className="bg-white rounded-xl shadow-sm p-6">
          ...
          <div className="flex justify-between p-4 bg-gray-50 rounded-lg">
            ...
            >
              {updating ? 'Updating...' : isAvailable ? 'Mark Unavailable' : 'Mark Available'}
            </button>
